...
def add_biomass(
    ...
    def ensure_biochar_store(n: pypsa.Network):
        ...
        """
        # Carrier
        if "biochar" not in n.carriers.index:
            n.add("Carrier", "biochar")

        # Bus
        if "biochar" not in n.buses.index:
            n.add("Bus", "biochar", carrier="biochar")

        # Store
        if "biochar" not in n.stores.index:
            n.add(
                "Store",
                "biochar",
                bus="biochar",
...
def add_pyrolysis(
    ...
    """
    add pyrolysis and new biochar store
        input: biomass
        output: electricity
        output: heat
        output: biochar
        output: neg.emissions

    n: network
    options: get options from config
    spatial : object
        Object containing spatial information about different carriers (gas, biomass, etc.)
    """

    if options["pyrolysis_multi-output"]["enable"]:
        include_stores = options["pyrolysis_multi-output"].get("include_stores", True) #if it is not in the config the default is True
        print("adding pyrolysis...")
        biomass_enabled = (
            options.get("biomass", False)
            or options.get("biomass_spatial", False)
            or options.get("biomass_transport", False)
        )
        # if biomass sector disabled; skip pyrolysis
        ...
        # Syngas Storage without hydrogen BUS
        ...
        ### stores 
        if include_stores:  #activate or deactivate in config
            n.add(
                "Store",
        ...
        n.add(
            ...
            bus0=spatial.biomass.df.loc[
                buses_i, "nodes"
            ].values,  # biomass input aligned with biomass nodes
            bus1="biochar",
            bus2=syngas1_buses_i,
            bus3="co2 atmosphere",
            carrier="pyrolysis",
            p_nom_extendable=True,
        ...
        if len(dh_locations) > 0:  #make sure that DH locations exist
            n.add(
                "Link",
                dh_locations + " syngas direct combustion for heat",  
                bus0=combu_direct_syngas_buses_i,   
                bus1=heat_buses_dh,  
                carrier="syngas direct combustion for heat",
                p_nom_extendable=True,
                marginal_cost=pyrolysis_data.loc[
                    "direct combu syngas", "marginal cost"
                ],
                capital_cost=pyrolysis_data.loc[
                    "direct combu syngas", "capital cost"
                ],
                efficiency=pyrolysis_data.loc[
                    "direct combu syngas", "efficiency"
                ],  # direct combustion of syngas
            )

        # Condensator (oil store vs syngas store)
        n.add(
            "Link",
            syngas2_buses_i + " condensator",
            bus0=syngas2_buses_i,
            bus1=oil_buses_i,  # to oil store
            bus2=syngas_store_buses_i,  # to syngas store
            p_nom_extendable=True,
            carrier="condensator",
            efficiency=pyrolysis_data.loc[
                "condensator", "efficiency-oil"
            ],  # to oil store
            efficiency2=pyrolysis_data.loc[
                "condensator", "efficiency-syngas"
            ],  # to syngas store
        )
        ...
        n.add(
            ...
            capital_cost=pyrolysis_data.loc["FT", "capital cost"],
        ...
        n.add(
            "Link",
            adsorbtion_buses_i,
            bus0=adsorbtion_buses_i,
            bus1=buses_i + " H2", 
            bus2=syngas_wo_h2_buses_i,  # syngas leftover without h2
            carrier="adsorbtion",
            p_nom_extendable=True,
            marginal_cost=pyrolysis_data.loc["adsorbtion", "marginal cost"],
            capital_cost=pyrolysis_data.loc["adsorbtion", "capital cost"],
            efficiency=pyrolysis_data.loc[
                "adsorbtion", "efficiency-h2"
            ],  # to h2 load direct
            efficiency2=pyrolysis_data.loc[
                "adsorbtion", "efficiency-syngas-wo-h2"
        ...
        if len(dh_locations) > 0: 
            n.add(
                "Link",
                dh_locations + " syngas cogen",  
                bus0=syngas_cogeneration_buses_i, 
                bus1=nodes,  #elec
        ...
        if len(dh_locations) > 0:  
            n.add(
                "Link",
                dh_locations + " syngas2 direct combustion for heat",  
                bus0=combu_direct_syngas_2_buses_i,  
                bus1=heat_buses_dh,  
        ...
        # ------------------------------------------------------------------
        syngas_wo_h2_cogeneration_buses_i = n.add(
            "Bus",
        ...
        if len(dh_locations) > 0: 
            n.add(
                ...
                p_nom_extendable=True,
                marginal_cost=pyrolysis_data.loc[
                    "syngas wo h2 cogen", "marginal cost"
                ],
                capital_cost=pyrolysis_data.loc[
                    "syngas wo h2 cogen", "capital cost"
                ],
                efficiency=pyrolysis_data.loc[
                    "syngas wo h2 cogen", "efficiency-elec"
                ],  # elec eff
                efficiency2=pyrolysis_data.loc[
                    "syngas wo h2 cogen", "efficiency-heat"
                ],  # heat eff
            )
...
if __name__ == "__main__":
    ...
    pyrolysis_data = (
    pd.read_csv('data/pyro_data.csv', index_col=0)
    .reset_index()
    .pivot(index='technology', columns='parameter', values='value')
    )
    ...
    #add_pyrolysis(n, spatial, options, pyrolysis_data, pop_layout, include_stores)
